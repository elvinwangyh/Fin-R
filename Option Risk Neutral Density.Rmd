...
option_exp.list <- names(all_chains)
expiration_date  <- toString(option_exp.list[chain_in_q]) 
...
## Call-Put Ratio
```{r}
chain$cp.ratio <- chain$calls.OI / chain$puts.OI
cp.ratio <- plot_ly(chain,
                    x = ~calls.Strike, y = ~cp.ratio,
...
chain$eu.mln.calls <- eu_mln_pricing$call
chain$eu.mln.puts <- eu_mln_pricing$put
...
### Stats
```{r}
eu_mln_lm_call <- lm(calls.Last~eu.mln.calls, data = chain)
eu_mln_lm_put <- lm(calls.Last~eu.mln.calls, data = chain)
...
chain$eu.gb.calls <- eu_gb_pricing$call
chain$eu.gb.puts <- eu_gb_pricing$put
...
### Stats
```{r}
eu_gb_lm_call <- lm(calls.Last~eu.gb.calls, data = chain)
eu_gb_lm_put <- lm(calls.Last~eu.gb.calls, data = chain)
...
chain$eu.ew.calls <- eu_ew_pricing$call
chain$eu.ew.puts <- eu_ew_pricing$put
...
### Stats
```{r}
eu_ew_lm_call <- lm(calls.Last~eu.ew.calls, data = chain)
eu_ew_lm_put <- lm(calls.Last~eu.ew.calls, data = chain)
...
for (i in seq_len(length(chain$calls.Strike))) {
    eu_shimko_call[i] <- price.shimko.option(k = chain$calls.Strike[i],
                                             r = rates$risk.free.rate,
                                             y = rates$dividend.yield,
                                             te = days_to_expiration / 365,
                                             s0 = spot_price,
...
for (j in seq_len(length(chain$calls.Strike))) {
    eu_shimko_put[j] <- price.shimko.option(k = chain$calls.Strike[j],
                                            r = rates$risk.free.rate,
                                            y = rates$dividend.yield,
                                            te = days_to_expiration / 365,
                                            s0 = spot_price,
...
chain$eu.shimko.calls <- eu_shimko_call
chain$eu.shimko.puts <- eu_shimko_put
...
### Stats
```{r}
eu_shimko_lm_call <- lm(calls.Last~eu.shimko.calls, data = chain)
eu_shimko_lm_put <- lm(calls.Last~eu.shimko.puts, data = chain)
...
### Pricing  
```{r, warning=FALSE}
chain$am.mln.calls <- numeric(length(chain$calls.Strike))
chain$am.mln.puts  <- numeric(length(chain$calls.Strike))

for (i in seq_len(length(chain$calls.Strike))) {
  if (chain$calls.Strike[i] < am_expected_f0) {
    chain$am.mln.calls[i] <- price.am.option(
    ...
    chain$am.mln.puts[i] <- price.am.option(
      k = chain$calls.Strike[i],
      r = rates$risk.free.rate,
      te = days_to_expiration / 365,
      w = ext_am_mln$w.2,
...
### Stats
```{r, echo=FALSE}
am_mln_lm_call <- lm(calls.Last~am.mln.calls, data = chain)
am_mln_lm_put <- lm(puts.Last~am.mln.puts, data = chain)
